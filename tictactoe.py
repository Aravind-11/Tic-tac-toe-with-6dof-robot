...
class Object_detect():
    ...
    def run(self):
        self.mc = MyCobot("/dev/tty.usbmodem14101", 115200)
...
def detect_color(frame, lower_color, upper_color, color_name, min_rect_size):
    ...
    #print(f"{color_name} red Centers:", color_centers)
...
def play_tic_tac_toe_with_camera(detect, cap):
    ...
    while True:
        ...
        if player_turn:
            cv2.putText(frame, "play your turn", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,0), 2, cv2.LINE_AA)
        else:
            cv2.putText(frame, "Cobot's turn", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,0), 2, cv2.LINE_AA)
        ...
        if key == ord(' ') and player_turn:  # Press 'Space' to make a move for Player X
            ...
            #result_image_red, red_centers = detect_color(image, lower_red, upper_red, "red", min_rect_size)
